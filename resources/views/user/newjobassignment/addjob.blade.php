    ...
    <main class="flex-1 bg-gray-100 overflow-y-auto">
        ...
        <div id="importModal"
            ...
            <div
                ...
                <!-- Download Template -->
                <a href="{{ asset('templates/Add_job_template_.xlsx') }}" download
        ...
        <div class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-[9999]">
            <div
                ...
                <div class="flex justify-between items-center px-4 md:px-6 py-3 border-b bg-blue-950">
                    ...
                    <a href="{{ route('addjob.user') }}" class="text-white hover:text-gray-200 transition text-xl">✖</a>
                </div>
                <!-- Body -->
                        ...
                        <table class="w-max md:w-full text-xs border-collapse">
                            <thead class="sticky top-0 bg-blue-50 shadow-sm ">
                                <tr class="text-sm text-center text-gray-700"
                                    ...
                                    <th class="px-2 py-3 border whitespace-nowrap">Estimated Transportation Cost
                                    </th>
                                    <th class="px-2 py-3 border whitespace-nowrap">Estimated Other Cost</th>
                            ...
                            <tbody class="divide-y">
                                ...
                                <tr class="hover:bg-gray-100 transition">
                                    ...
                                    <td class="px-2 py-1 border text-end whitespace-nowrap text-[14px]">
                                        {{ isset($data['Estimated_Revenue']) ? number_format($data['Estimated_Revenue'],
                                        2) : '-' }}
                                    </td>
                                    <td class="px-2 py-1 border text-end whitespace-nowrap text-[14px]">
                                        {{ isset($data['Estimated_Service_Cost']) ?
                                        number_format($data['Estimated_Service_Cost'], 2) : '-' }}
                                    </td>
                                    <td class="px-2 py-1 border text-end whitespace-nowrap text-[14px]">
                                        {{ isset($data['Estimated_Material_Cost']) ?
                                        number_format($data['Estimated_Material_Cost'], 2) : '-' }}
                                    </td>

                                    <td class="px-2 py-1 border text-end whitespace-nowrap text-[14px]">
                                        {{ isset($data['Estimated_Transportation_Cost']) ?
                                        number_format($data['Estimated_Transportation_Cost'], 2) : '-' }}
                                    </td>
                                    <td class="px-2 py-1 border text-end whitespace-nowrap text-[14px]">
                                        {{ isset($data['Estimated_Other_Cost']) ?
                                        number_format($data['Estimated_Other_Cost'], 2) : '-' }}
                                    </td>


                                    <td class="px-2 py-1 border text-end whitespace-nowrap text-[14px]">
                                        {{ isset($data['Estimated_Gross_Profit']) ?
                                        number_format($data['Estimated_Gross_Profit'], 2) : '-' }}
                                    </td>
                                    <td class="px-2 py-1 border text-center whitespace-nowrap text-[14px]">
                                        {{ isset($data['Estimated_Gross_ProfitMargin']) ?
                                        number_format($data['Estimated_Gross_ProfitMargin'], 2) : '-' }}%
                                    </td>
                ...
                <div class="flex justify-end items-center px-6 py-2 border-t bg-gray-50">
                    ...
                    <form action="{{ route('addjob.saveimportnewjob') }}" id="save" method="POST"
                        ...
                        <input type="hidden" name="dataToSave[{{ $index }}][Estimated_Transportation_Cost]"
                            value="{{ $data['Estimated_Transportation_Cost'] ?? '' }}">
                        <input type="hidden" name="dataToSave[{{ $index }}][Estimated_Other_Cost]"
                            value="{{ $data['Estimated_Other_Cost'] ?? '' }}">
            ...
            <div
                ...
                <form action="{{ route('addjob.savenewjob') }}" method="POST" class="space-y-4" autocomplete="off">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-2">
                        ...
                        <div>
                            <label class="block mb-1 font-sarabun">Estimated Revenue <span class="text-red-500">
                                    *</span></label>
                        ...
                        <div>
                            <label class="block mb-1 font-sarabun">Estimated Service Cost <span class="text-red-500">
                                    *</span></label>
                        ...
                        <div>
                            <label class="block mb-1 font-sarabun">Estimated Material Cost <span class="text-red-500">
                                    *</span></label>
                        ...
                        <!-- เพิ่มเติม -->
                        <div>
                            <label class="block mb-1 font-sarabun">Estimated Transportation Cost<span
                                    class="text-red-500"> *</span></label>
                            <input type="text" id="estimated_transportation_cost" name="estimated_transportation_cost"
                                class="border rounded-md w-full p-1" required>
                        </div>

                        <div>
                            <label class="block mb-1 font-sarabun">Estimated Other Cost<span class="text-red-500">
                                    *</span></label>
                            <input type="text" id="estimated_other_cost" name="estimated_other_cost"
                                class="border rounded-md w-full p-1" required>
                        </div>
        ...
        <div class="bg-white p-4 shadow-md h-[560px]">
            ...
            <div class="relative overflow-x-auto mt-2 overflow-y-auto h-[400px] font-sarabun">
                <table id="table" class="min-w-max table-fixed border-separate border-spacing-0
                                [--th-h:20px]
                                [--th-w:20px]
                                [--th-px:6px]
                                [--th-py:2px]
                    ...
                    <tbody>
                        ...
                        <tr class="hover:bg-red-100 group font-sarabun transition-colors duration-200 text-xs ">
                            <td
                                    ...
                                    <div
                                        ...
                                        <form action="{{ route('update.job.status', $item->id) }}" method="POST"
                                            ...
                                            <button type="submit" name="Job_Adding_Status" value="{{ $status }}"
                                                ...
                                                <span class="w-1 h-1 {{ $c['dot'] }} rounded-full"></span>
                                                {{ $status }}
                                            </button>
                                            @endforeach
                                        </form>
        ...
        text-xs font-sarabun px-2 py-1 rounded-full">
            ...
            rounded-full">
                            ...
                            <td class="truncate-cell border-b whitespace-nowrap text-left" onclick="toggleExpand(this)">
                                {{ $item->Site_Name }}
                            </td>
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
            ...
            const transportationInput = document.getElementById('estimated_transportation_cost');
            const otherInput = document.getElementById('estimated_other_cost');
            ...
            const modalInputs = [revenueInput, serviceCostInput, materialCostInput, transportationInput,
                otherInput, grossProfitInput,
                grossProfitMarginInput
            ];

            // คำนวน Gross Profit และ Gross Profit Margin
            function calculateGross() {
                ...
                const transportationCost =
                    parseFloat(transportationInput.value.replace(/,/g, '')) || 0;
                const otherCost =
                    parseFloat(otherInput.value.replace(/,/g, '')) || 0;

                const grossProfit =
                    revenue - serviceCost - materialCost - transportationCost - otherCost;
            ...
            [revenueInput,
                ...
                input.addEventListener("input", (e) => {
                    ...
                    let newCursor =
                        cursor +
                        ((formatted.match(/,/g) || []).length - commasBefore);
...
<script>
    function exportToExcel() {
        ...
        XLSX.writeFile(wb, "AddJob_sda.xlsx");
    }
</script>
